...
start_state = "812043765"
goal_state = "012345678"

if is_solvable(start_state):
    start_time1 = time.time()
    _, res, nodes_expanded = bfs_solver(start_state, goal_state)
    end_time1 = time.time()
    cost = res[0]
    path = res[1]
    print(RED + "BFS" + RESET)
    ...
    _, res, nodes_expanded = dfs_solver(start_state, goal_state)
    end_time2 = time.time()
    cost = res[0]
    path = res[1]
    print(RED + "DFS" + RESET)
    ...
    _, res, nodes_expanded = astar(start_state, goal_state, "manhatan")
    end_time3 = time.time()
    cost = res[0]
    path = res[1]
    ...
    _, res, nodes_expanded = astar(start_state, goal_state, "eucledian")
    end_time4 = time.time()
    cost = res[0]
    path = res[1]
